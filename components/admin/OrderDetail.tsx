...
import {
  ...
  CreditCard, FileText, Users2, GraduationCap, Clock, UserPen } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { StatusBadge, Badge } from '@/components/ui/Badge';
import { DeadlineMeter, deadlineState } from '@/components/ui/DeadlineMeter';
import { Skeleton, EmptyState, ErrorState } from '@/components/ui/States';
import { useAsync } from '@/hooks/useAsync';
import { EditStudentProfileDialog } from '@/components/admin/students/EditStudentProfileDialog';
...
export function OrderDetail({
  orderId, isSuperAdmin, canEditStudent = false, onBack, onAssign, onSetPrice, onPayLink, onOpenFiles,
  ...
  // The h1 below lives in the success render, so the loading and error branches
  // would leave this route with no heading at all. A hidden one keeps the page
  // to exactly one h1 in every state.
  if (detail.error) {
    return (
      <>
        <h1 className="visually-hidden">Order detail</h1>
      ...
      <div className="od-head">
        <div>
          ...
          <div className="od-title">
            {/* The order number is this page's title. It was a <span>, which
                left /admin/orders/[id] with no h1 at all once the detail view
                became its own route. Styling is unchanged — .od-id carries it. */}
            <h1 className="od-id">{fmtOrderId(d.id)}</h1>
        ...
        <div className="od-rail">
          ...
          <section className="card">
            <div className="card-head">
              ...
              {canEditStudent && d.studentId != null && (
                <Button size="sm" variant="ghost" onClick={() => setEditingStudent(true)}>
            ...
            {canEditStudent && d.studentId != null && (
              <EditStudentProfileDialog
                ...
                current={{ name: d.studentName, email: d.studentEmail, phone: d.studentPhone }}
                onClose={() => setEditingStudent(false)}
                onSaved={(saved) =>
                  /* Patched from the PUT response, not re-fetched: there is no
                     GET for a single student, and this order's embedded student
                     fields could be a copy taken when the order was placed — a
                     refetch might put the old values straight back. */
                  detail.setData((prev) =>
                    prev ? {
                      ...prev,
                      studentName: saved.name,
                      studentEmail: saved.email,
                      studentPhone: saved.phone || null,
                    } : prev
                  )
                }
              />
