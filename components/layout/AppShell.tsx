...
export interface NavItem {
  ...
  icon: LucideIcon;
  /**
   * The item's route. When set, the item renders as a real link, so each
   * sidebar destination is its own URL and history entry — Back and Forward
   * move between pages, and middle-click opens a page in a new tab.
   */
  href?: string;
  superOnly?: boolean;
  count?: number;
...
interface AppShellProps {
  ...
  groups: NavGroup[];
  active: string;
  /** Only used by items without an `href`. */
...
/** A route item renders as a link; anything without a route falls back to a button. */
function NavEntry({
  item, on, rail, onNavigate, onPick, children,
}: {
  item: NavItem;
  on: boolean;
  rail: boolean;
  onNavigate?: (key: string) => void;
  /** Closes the mobile drawer. Route changes close it anyway; this covers
   *  tapping the page you are already on, where the route does not change. */
  onPick?: () => void;
  ...
  if (item.href) {
    return <Link href={item.href} {...common} onClick={onPick}>{children}</Link>;
  }
  return (
    <button type="button" {...common} onClick={() => onNavigate?.(item.key)}>
      {children}
    </button>
  );
}
