    ...
    <>
      ...
      <header className="lp-header">
        ...
        <nav className="lp-nav" aria-label="Main">
          <ul>
            {/* Anchor entries scroll within the current page; `route` entries
                are real pages and use next/link, so they also work correctly
                from the legal pages where a bare `#services` would not. */}
            {NAV.map((n) => (
              <li key={n.href}>
                {'route' in n && n.route
